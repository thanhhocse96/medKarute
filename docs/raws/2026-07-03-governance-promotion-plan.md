...
|---|---|---|---|---|---|
| `README.md` | §7 bổ sung scope (user chốt 2026-07-03) | Orientation kỹ thuật root — **EN canonical** | **1** Giới thiệu 1 đoạn. **2** Bắt đầu (coding tool hoặc CLI, "bắt đầu" → onboarding). **3** Mermaid workflow sơ lược + link `00-overview.md`. **4** Bảng Công cụ & vai trò (5 hàng). **5** Link nhanh AGENTS/CLAUDE/overview/endnote-workflow. Link → `docs/readme/` | ~60–90 dòng | Không thay `huong-dan-su-dung.md`; không lặp `AGENTS.md`/`00-overview.md` |
| `docs/readme/README.vi.md` | Bản dịch từ `README.md` | Orientation VI — **ngoài agent load map** | Cùng 5 mục; thư mục `docs/readme/` không liệt kê trong GLOBAL active docs / AGENTS §1 | ~60–90 dòng | Locale sau (hi, …) cùng thư mục; index human: `docs/readme/README.md` |
...
## 7. Bổ sung scope — root `README.md` (user chốt 2026-07-03)

**Vấn đề**: repo chưa có `README.md` ở root — người mở repo lần đầu (kể cả agent coding tool khác: Grok, Cursor, Codex...) không có điểm vào duy nhất để hiểu "đây là gì, dùng sao". Khác `huong-dan-su-dung.md` (đã defer G3 — distill sâu cho end-user bác sĩ dùng sau khi guide có nội dung thật) — file này là **README kỹ thuật chung**, ngắn, orientation-level.

**Nội dung bắt buộc**:

1. **1 đoạn giới thiệu** — research-helper là gì (chat-driven research assistant, Claude/agent orchestrate 2 MCP)
2. **Hướng dẫn sử dụng ngắn** — cách bắt đầu: mở repo bằng agent coding tool (Claude Code, Grok CLI, Cursor...) HOẶC dùng thẳng CLI của agent đó, nói "bắt đầu" → onboarding tự chạy. **Nhấn mạnh: không có gì phức tạp** — agent coding tool ở đây chỉ để **nhìn rõ file/diff/tree** dễ hơn, bản chất vẫn là agent đọc `AGENTS.md`/`CLAUDE.md` + file `.md`; dùng thuần CLI (`claude`, `grok` command line, không mở IDE) **vẫn chạy được y hệt** — không phụ thuộc tool cụ thể nào.
3. **Workflow sơ lược** (1 mermaid flowchart ngắn — không lặp lại chi tiết `00-overview.md`): PDF/chat → paper note/session → insight → writing, có EndNote ở giữa. Link `docs/guides/research/00-overview.md` cho chi tiết.
4. **Bảng "Công cụ & vai trò"** — liệt kê từng thành phần bên ngoài repo và vai trò nó đóng, để người đọc lần đầu (hoặc agent khác) không phải suy đoán:
...
| **whyschools** (nguồn brainstorm gốc, xem `docs/raws/research-helper.md`) | Blog nguồn cảm hứng ban đầu cho ý tưởng kết hợp MarkItDown + endnote-mcp — **không phải dependency chạy**, chỉ là nguồn tham khảo thiết kế |
| **context-mapping pattern** (từ `skvn-marine`, xem `docs/raws/agent-memory-and-load-protocol.md` §0) | Kiến trúc `.context/` (GLOBAL/MILESTONES/TENSIONS/modules) — mượn nguyên pattern quản lý AI memory, **không phải tool cài đặt**, là convention áp dụng |
| **Markpad** | App mở file `.md` local cho user xem note — không phải MCP, chỉ là viewer |

5. **Link nhanh**: `AGENTS.md` (invariants), `CLAUDE.md` (playbook), `docs/guides/research/00-overview.md` (chi tiết workflow), `docs/decisions/endnote-workflow.md` (EndNote canonical)

**Vị trí**: `README.md` root — ngoài bảng §1.1 gốc (bổ sung thêm, không phải outline cũ). Ngôn ngữ VI + thuật ngữ EN, khớp deferred-gaps G2.

**Việc Grok cần làm**:
1. Viết `README.md` root theo 5 mục trên — ngắn, không lặp lại nội dung đã có trong `00-overview.md`/`AGENTS.md`, chỉ orientation + link
2. Thêm `README.md` vào bảng §1.1 của plan này (cập nhật outline nếu cần)

**Đã làm — 2026-07-03 (Grok)**: `README.md` root + dòng §1.1 plan.  
**Đã làm — 2026-07-03 (Grok)**: `README.md` EN canonical; `docs/README.vi.md` + cross-link.  
**Đã làm — 2026-07-03 (Grok)**: Chuyển locale → `docs/readme/`; exclude khỏi AGENTS §1 + GLOBAL active docs.
...
**Đã làm — 2026-07-03 (Grok, T-001 resolution)**: User quyết bỏ wrapper — thực thi 6 việc trong `.context/TENSIONS_ACTIVE.md` T-001: xóa `tools/mcp-wrappers/run-endnote-mcp.sh` + `.cmd`; `.mcp.json` trở về bare `uvx endnote-mcp serve`; viết lại `docs/decisions/endnote-workflow.md §MCP config resolution` (2-lựa-chọn onboarding: native wizard vs agent); thêm `setup_method` vào schema + `.local/mcp/endnote.md`; cập nhật `CLAUDE.md §G` + `docs/guides/mcp/endnote-mcp-tools.md` Prerequisite; pointer `.local/mcp/endnote.md` + `claude-agent-summary.md` trỏ decision mới. QA4 wrapper (§5) **đã supersede** bởi T-001 — giữ lịch sử §5, không xóa.
